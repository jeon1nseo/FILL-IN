...
def save_config(roi):
    """현재 캘리브레이션(ROI·회전·튜닝값)을 파일로 저장."""
    cfg = {
        "roi": [int(v) for v in roi],
        "rotate": ROTATE_STATE,
        "v_min": WHITE_V_MIN,
        "row_ratio": ROW_FILL_RATIO,
    }
    with open(CONFIG_PATH, "w") as f:
        json.dump(cfg, f, indent=2)
    print(f"[CONFIG] 설정 저장됨: {CONFIG_PATH} → {cfg}")
...
class FillDetector:
    """
    통합용 클래스 (창·마우스 없이 동작).
    캘리브레이션(fill_config.json)을 불러와서, is_full()을 부르면
    '용기가 다 찼는지'를 True/False 로 반환한다.

    사용 예 (통합 담당):
        from fill_monitor import FillDetector
        det = FillDetector()          # 저장된 설정 불러오고 카메라 염
        while ...:
            full = det.is_full()      # True / False
            # 이 값을 VIS 프레임의 플래그로 실어 아두이노에 전송
        det.release()
    """

    def __init__(self, config_path=CONFIG_PATH):
        cfg = load_config() if os.path.exists(config_path) else json.load(open(config_path))
        self.roi = tuple(cfg["roi"])
        self.rotate = cfg["rotate"]
        self.v_min = cfg["v_min"]
        self.row_ratio = cfg["row_ratio"]
    ...
    def is_full(self):
        """용기가 FILL_THRESHOLD% 이상 CONFIRM_FRAMES 연속 → True."""
        ret, frame = self.cap.read()
        if not ret:
            return False
        if self.rotate in _ROTATE_CODES:
            frame = cv2.rotate(frame, _ROTATE_CODES[self.rotate])

        fill, _, _, _ = measure_fill(frame, self.roi, self.v_min, self.row_ratio)
        if fill >= FILL_THRESHOLD:
            self.reached += 1
        else:
            self.reached = 0
        return self.reached >= CONFIRM_FRAMES
...
def measure_fill(frame, roi, v_min=None, row_ratio=None):
    """
    ROI 안에서 '흰색 내용물'이 어디까지 찼는지로 충진율(%)을 계산.
    - HSV로 바꿔 '밝고(V높음) 색이 옅은(S낮음)' 픽셀 = 흰색 내용물로 판단
    - 위에서 아래로 스캔, 흰 픽셀이 충분히 많은 첫 줄 = 내용물 표면
    반환: (충진율%, 표면 y좌표, 디버그용 마스크, 선명도)
    v_min/row_ratio 를 넘기면 그 값을, 안 넘기면 전역 기본값을 사용.
    """
    if v_min is None:
        v_min = WHITE_V_MIN
    ...
    mask = cv2.inRange(hsv, (0, 0, v_min), (180, WHITE_S_MAX, 255))
...
def main():
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            save_config(roi)   # 현재 캘리브레이션을 파일로 저장 (통합용)
        elif key == ord('r'):
...
def run_headless():
    """통합 경로 테스트: 저장된 설정으로 is_full()을 반복 출력 (창·마우스 없음)."""
    if not os.path.exists(CONFIG_PATH):
        print(f"설정 파일이 없습니다. 먼저 캘리브레이션(python3 fill_monitor.py)으로 "
              f"ROI 잡고 'w'로 저장하세요.")
        return
    det = FillDetector()
    print("통합 테스트 시작 (Ctrl+C로 종료). is_full() 결과를 계속 출력합니다.")
    try:
        while True:
            print("FULL" if det.is_full() else "...")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'run':
        run_headless()      # 통합용(비대화형) 테스트
    else:
        main()              # 캘리브레이션/모니터링(대화형)
